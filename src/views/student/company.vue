  ...
  <div class="container">
    ...
    <el-table v-loading="loading" :data="tableData" style="width: 100%; margin-top: 20px;" border>
      ...
      <el-table-column prop="established_date" label="建立日期" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.established_date ? new Date(scope.row.established_date).toLocaleDateString() : '' }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="简介预览" :visible.sync="previewVisible" width="50%">
      <div style="height: 400px; overflow-y: auto; border: 1px solid #ccc; padding: 10px;margin-top: -20px;" v-html="dialogForm.introduction" />
