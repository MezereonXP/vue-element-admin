<template>
  <div class="container">
    <h1 style="text-align: center;">法律法规文件制度</h1>
    ...
    <el-table :data="tableData" style="margin-top: 20px;width: 60%; margin: 0 auto;">
      ...
      <el-table-column prop="title" label="标题" width="1000">
        <template slot-scope="scope">
          <router-link :to="`/law/detail/${scope.row.id}`" style="color: #000; text-decoration: none;">{{ scope.row.title }}</router-link>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      text_type: 2, // Text type for the TinyMCE editor
      page: 1,
      limit: 10,
      total: 0,
      title: ''
