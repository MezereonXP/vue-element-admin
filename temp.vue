    ...
    <div class="container">
        <el-card class="card-container">
            <div slot="header" class="card-header">
                ...
                <el-button v-if="isAdmin" type="success" size="small" icon="el-icon-plus" plain class="add-button"
                    @click="openEditorForAdd">添加</el-button>
            </div>
            ...
            <el-table :data="tableData" style="width: 100%;">
                ...
                <el-table-column v-if="isAdmin" label="操作" min-width="15%">
                    <template slot-scope="scope">
                        <el-button v-if="isAdmin" type="primary" size="small" icon="el-icon-edit" plain
                            @click="openEditor(scope.row)" />
                        <el-button v-if="isAdmin" type="danger" size="small" icon="el-icon-delete" plain
            ...
            <el-pagination background
                :layout="isMobileView ? 'prev, pager, next' : 'total, sizes, prev, pager, next, jumper'" :total="total"
                :page-size="limit" :current-page="page" class="pagination" @current-change="handleCurrentChange"
                @size-change="handleSizeChange" @prev-click="handlePrevClick" @next-click="handleNextClick" />
        </el-card>

        <el-dialog :visible.sync="editorVisible" :title="transTitle" custom-class="custom-dialog"
            :width="isMobileView ? '95%' : '60%'" :close-on-click-modal="false" :before-close="handleCloseDialog">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" :loading="submitLoading"
                    @click="transTitle === '编辑' ? saveContent() : addContent()">{{ transTitle === '编辑' ? '保存' : '添加'
                    }}</el-button>
            </span>
        </el-dialog>
    </div>
